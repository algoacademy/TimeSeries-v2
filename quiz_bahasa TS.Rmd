...
4. Untuk membuat model *Holt-Winters*, manakah *code* yang paling tepat untuk memodelkan data `train_assault`?
 ...
 - [ ] HoltWinters(train_theft)
 - [ ] HoltWinters(train_theft, beta = F)
 - [ ] HoltWinters(train_theft, beta = F, gamma = F)
___

Mari kita mengekplorasi model lain untuk meramalkan data `train_assault` menggunakan algoritma ARIMA. Silahkan bangun sebuah model ARIMA menggunakan fungsi `stlm()`. Aturlah argumen `method = "arima"` dan argumen `s.window` berdasarkan frekuensi objek time series yang digunakan sebelumnya. Simpan hasilnya ke dalam objek `model_arima`.
...
6. Berdasarkan hasil di atas, manakah dari pernyataan berikut yang **BENAR**?
  - [ ] Nilai *mean absolute percentage error* pada model ARIMA sebesar 17.01%
